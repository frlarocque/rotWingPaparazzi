...
void ekf_aw_propagate(struct FloatVect3 *acc,struct FloatRates *gyro, struct FloatEulers *euler, float *pusher_RPM,float *hover_RPM_array, float *skew, float *elevator_angle, FloatVect3 * V_gnd, FloatVect3 *acc_filt, float *V_pitot,float dt)
{
  ...
  */
  ...
  eawp.inputs.RPM_hover = Vector4f(hover_RPM_array[0],hover_RPM_array[1],hover_RPM_array[2],hover_RPM_array[3]); // TO DO: Crashes filter for some reason?
  ...
  */
  ...
  a_y = beta*ekf_aw_params.k_beta*(V_a*V_a) + eawp.state.offset(1); // No need to bound beta equation as beta is generated by asin, which is bounded between -pi/2 and pi/2
  ...
  */
  ...
  G(4,0) = 2*ekf_aw_params.k_beta*u*beta - (ekf_aw_params.k_beta*u*v)/(cos_beta*protected_V_a);
  G(4,1) = 2*ekf_aw_params.k_beta*v*beta + (ekf_aw_params.k_beta*(V_a - v*sinf(beta)))/cos_beta;
  G(4,2) = 2*ekf_aw_params.k_beta*w*beta - (ekf_aw_params.k_beta*v*w)/(cos_beta*protected_V_a);
  G(4,7) = 1;
